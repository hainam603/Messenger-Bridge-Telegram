...
- `fbchat-v2` cài từ PyPI, GitHub repo hoặc source local.
- Go 1.24 trở lên nếu cần build E2EE binary.
- Git nếu muốn cài `fbchat-v2` từ GitHub hoặc clone source để build E2EE.
...
Sau đó chọn cách cài `fbchat-v2` ở phần tiếp theo. Nếu chưa rõ chọn gì, dùng source local hoặc cài trực tiếp từ GitHub repo để có code mới nhất.

## Cài fbchat-v2

Bridge hỗ trợ ba cách dùng `fbchat-v2`. Chỉ nên chọn một cách cho một môi trường chạy.

### Cách 1: Cài từ PyPI

Đây là cách gọn nhất khi bản PyPI đã đủ tính năng bạn cần.

```powershell
cd "c:\Users\minhh\Downloads\mhwidev - project\Messenger-Bridge-Telegram"
.\.venv\Scripts\Activate.ps1
python -m pip install --upgrade fbchat-v2
```
...
Tên biến là `FBCHAT_V2_USE_PYPI`, nhưng ý nghĩa thực tế là dùng package đã cài trong Python environment thay vì chèn folder source local vào `sys.path`.

Kiểm tra import:

```powershell
python -c "from importlib import import_module; print(import_module('fbchat_v2._core._session').__file__)"
```

### Cách 2: Cài trực tiếp từ GitHub repo

Cách này vẫn chạy ở package mode như PyPI, nhưng lấy code từ repository. Phù hợp khi PyPI chưa publish bản mới.
...
### Cách 3: Dùng source local cạnh project

Cách này phù hợp khi bạn đang sửa `fbchat-v2`, cần build E2EE binary, hoặc muốn dùng các patch trong workspace ngay lập tức.

Nếu workspace đã có thư mục `fbchat-v2` nằm cạnh `Messenger-Bridge-Telegram`, bridge sẽ tự detect:
...
```env
FBCHAT_V2_USE_PYPI=0
# FBCHAT_V2_SRC_PATH=../fbchat-v2/src
```

Nếu thư mục nằm chỗ khác, set thủ công:
...
Clone source local từ GitHub nếu chưa có:
...
Package PyPI/GitHub có thể cung cấp Python module, nhưng không phải lúc nào cũng kèm binary Go. Nếu `FBCHAT_ENABLE_E2EE=1`, bạn nên tự build hoặc trỏ tới binary đã build sẵn bằng `FBCHAT_E2EE_BIN`.
...
| `FBCHAT_V2_USE_PYPI` | Không | `0` | Dùng package đã cài từ PyPI/GitHub thay vì source local. |
| `FBCHAT_V2_SRC_PATH` | Không | auto-detect | Trỏ tới `fbchat-v2/src` khi dùng source local. |
| `FBCHAT_E2EE_BIN` | Không | auto-detect local | Đường dẫn tới `fbchat-bridge-e2ee[.exe]`. Nên set rõ khi dùng package mode. |
...
- Nếu dùng package mode từ PyPI/GitHub, vẫn nên set `FBCHAT_E2EE_BIN` rõ ràng khi bật E2EE.
...
Nếu dùng PyPI hoặc cài từ GitHub:
...
Kiểm tra import package mode:

```powershell
python -c "from importlib import import_module; print(import_module('fbchat_v2._messaging._listening_e2ee').__file__)"
```

Kiểm tra local source mode:

```powershell
$env:PYTHONPATH="..\fbchat-v2\src"
python -c "from importlib import import_module; print(import_module('_messaging._listening_e2ee').__file__)"
```
